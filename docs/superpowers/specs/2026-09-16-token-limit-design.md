...
Часовые пояса: бэкенд считает всё в одной таймзоне — UTC (aware-даты через
`utcnow()`, не зависит от TZ сервера); `resets_at` всегда отдаётся с `Z`. Фронт
показывает время сброса только в местном времени браузера, «сегодня/другой день» —
тоже по местной дате.
